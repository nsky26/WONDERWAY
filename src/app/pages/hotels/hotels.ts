...
export class HotelsComponent implements OnInit {
  ...
  doInlineSearch(): void {
    if (!this.inlineSearch.to || !this.inlineSearch.date) return;
    this.searchParams = { from: '', ...this.inlineSearch };
    this.hasSearched = true;
    this.performSearch();
  }
  ...
  calculatePrice(): number {
    if (this.selectedHotel) {
      ...
      return this.selectedHotel.pricePerNight * days;
    }
    return 0;
  }
  ...
  getDaysDifference(): number {
    if (this.searchParams.date && this.searchParams.checkoutDate) {
      const checkIn = new Date(this.searchParams.date);
  ...
  onSubmit() {
    ...
    if (!this.selectedHotel) {
      this.errors['form'] = 'Please select a hotel';
      return;
    }
    ...
    const bookingData = {
      id: bookingId,
      bookingId: bookingId,
      destinationId: this.selectedHotel.id,
      destinationName: this.selectedHotel.name,
      ...
      selectedItem: this.selectedHotel,
      to: this.selectedHotel.city,
    ...
    this.pdfService.generateBookingPDF(bookingData, 'Hotel');
  ...
  private generateBookingId(): string {
    ...
    return `WW-HTL-${timestamp}${random}`;
  }
  ...
  downloadPDFAgain(): void {
    if (this.bookingConfirmation) {
      this.pdfService.generateBookingPDF(this.bookingConfirmation, 'Hotel');
    }
  }
